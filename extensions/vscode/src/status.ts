...
export class StatusBar {
  ...
  idle(snapshot: Snapshot | undefined, warnWithinDays: number): void {
    if (!snapshot) {
      ...
      this.item.tooltip = 'Nothing collected yet — click to open the report, or run "expiry-radar: Refresh Inventory".';
      this.item.backgroundColor = undefined;
      this.item.show();
      return;
    }
...
function warningBackground(snapshot: Snapshot): vscode.ThemeColor | undefined {
  return snapshot.warnings.length ? new vscode.ThemeColor("statusBarItem.warningBackground") : undefined;
}

...
function published(publisher: DiagnosticPublisher): Map<string, Diagnostic[]> {
  return (publisher as unknown as { collection: { _entries: Map<string, Diagnostic[]> } }).collection._entries;
}
...
test("severity follows the deadline, on the configured windows", () => {
  ...
  publisher.publish([
    {
      items: [item({ daysLeft: -1 }), item({ daysLeft: 5 }), item({ daysLeft: 20 }), item({ daysLeft: 90 })],
      settings: settings(),
    },
  ]);
  assert.deepEqual(
...
test("an item nothing in the repository declared gets no squiggle", () => {
  const publisher = new DiagnosticPublisher();
  publisher.publish([{ items: [item({ origin: undefined, source: "k8s:ingress" })], settings: settings() }]);
...
test("two folders publish into one collection without displacing each other", () => {
  ...
  assert.deepEqual([...published(publisher).keys()].sort(), ["/other/expiry-radar.json", "/repo/expiry-radar.json"]);
  publisher.dispose();
});

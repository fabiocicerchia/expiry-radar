...
test("adding an endpoint prompts for it, writes it, and opens the file it wrote", async () => {
  ...
  try {
    await activated(async () => {
      ...
      answers.push((items: { label: string }[]) => items.find((i) => i.label === "Endpoint"), REFUSED);
      ...
      const written = JSON.parse(fs.readFileSync(path.join(project, "expiry-radar.json"), "utf8")) as {

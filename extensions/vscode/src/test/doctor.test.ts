...
function fixture(usage: string, config?: unknown): { folder: never; binary: string; dir: string } {
  ...
  if (config !== undefined) {
    fs.writeFileSync(path.join(dir, "expiry-radar.json"), typeof config === "string" ? config : JSON.stringify(config));
  }
...
test("something on disk that is not expiry-radar is named, not trusted", async () => {
  ...
  try {
    ...
    assert.equal(summary, "expiry-radar: the doctor found something that will limit results — see the log.");
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
...
test("a config that enables nothing at all is an error, and the summary says so", async () => {
  ...
  try {
    ...
    assert.equal(summary, "expiry-radar: the doctor found something that stops it running — see the log.");
  } finally {
    fs.rmSync(dir, { recursive: true, force: true });
  }
});
